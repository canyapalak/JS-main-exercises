...
let myClassNames = ["Can", "Juli", "Santiago", "Olja", "Ercan", "Monique", "Roxanne", "Raul"];
myClassNames.sort();
...
while (j < myClassAges.length) {
    if (myClassAges[j].age % 2 == 0) {
        console.log(myClassAges[j].name, "'s age is", myClassAges[j].age, "and it is an EVEN number!");
    } else {
        console.log(myClassAges[j].name, "'s age is", myClassAges[j].age, "and it is an ODD number!");
    }
    j++;
}
...
for (let k = 0; k < myClassAges.length; k++) {
    if (myClassAges[k].age % 2 == 0) {
        console.log(myClassAges[k].name, "'s age is", myClassAges[k].age, "and it is an EVEN number!");
    } else {
        console.log(myClassAges[k].name, "'s age is", myClassAges[k].age, "and it is an ODD number!");
    }
}
...
function least_number(myClassAges) {
    ...
    for (let x = 0; x < myClassAges.length; x++) { //comparison age vs 100
...
function highest_number(myClassAges) {
    for (let x = 0; x < myClassAges.length; x++) { //comparison age vs 0
...
const indexNumber = 5
const numberArray = [25, 183, 1, 6, 84, 41, 2, 1, 407, 6, 643, 1, 25, 33, 9, 72];
...
console.log(indexNumberPlusOne + "th element of the array is " + positionOfAnElement(numberArray));
...
function reversedNum(randomNumber) {
    let reversed = parseFloat(randomNumber.toString().split('').reverse().join(''))
    return reversed
}

console.log("Reversed version of " + randomNumber + " is " + reversedNum(randomNumber))
...
console.log("Alphabetical order of the letters in 'Hello World' is: " + returnStringIntoAlphabet("Hello World"));
...
console.log(capitalizeFirstLetters('first letters of this sentence are uppercase'));
...
console.log("Longest word: " + findLongestWord("When life gives you lemons, make lemonade"));
...
console.log(addBands(["Depeche Mode", "Pet Shop Boys", "The Smiths", "Kraftwerk"]));
